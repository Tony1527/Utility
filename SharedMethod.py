...
def MultiOperObj(p_list,*arg,**kwargs):
    ...
    for i in range(len(p_list)):
        p=p_list[i]
        obj=kwargs["class"](*p)
        rtn.append(OperObj(obj,kwargs["operator"],arg,kwargs))  #could be wrong when shallow copy is used
        del obj
        end=time.time()
        Info("============PID %d completes (%d/%d) jobs cost %fs"%(os.getpid(),i+1,len(p_list),end-begin))
...
def MultiprocTask(p_list,func,process_num=2,is_self_unzip=True,*arg,**kwargs):
    ...
    Success("PID:"+str(os.getpid())+" MultiprocGetE_k using %d Process cost %fs"%(process_num,end-begin))
    return rtn